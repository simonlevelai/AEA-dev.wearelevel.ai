...
/**
 * Local Server Demo for M365 Agents SDK Multi-Agent System
 * Provides web interface to test the healthcare chatbot
 */
...
// Mock multi-agent system (using our working demo logic)
class MockHealthcareAgents {
  private logger: Logger;

  constructor() {
    this.logger = new Logger('healthcare-agents');
  }

  async processHealthcareMessage(userMessage: string): Promise<{
    success: boolean;
    response: string;
    agentUsed: string;
    responseTime: number;
    ...
    this.logger.info('Processing healthcare message through M365 Agents SDK', {
    ...
    if (lowerMessage.includes('hopeless') || lowerMessage.includes('end my life') || 
      ...
      await this.delay(400); // <500ms crisis response
...
For questions about gynaecological cancers, here are some key points:
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    logger.error('Chat API error', { error: error instanceof Error ? error : new Error(String(error)) });
    res.status(500).json({ 
...
<body>
    ...
    <script>
        ...
        function addMessage(text, isUser, agentName, responseInfo) {
            ...
            if (responseInfo) {
                if (responseInfo.suggestedActions && responseInfo.suggestedActions.length > 0) {
...
// Start server
app.listen(PORT, () => {
  logger.info(`🚀 Ask Eve Assist M365 Agents SDK Demo Server started`, {
    port: PORT,
    url: `http://localhost:${PORT}`,
    architecture: 'Multi-Agent Healthcare System',
    framework: 'Microsoft 365 Agents SDK 2025'
  });
